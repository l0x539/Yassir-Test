...
const getApiPokemonByName = (ApiPokemanPath, headers, method="GET", body=undefined) => {
...
export const getPokemonByName = async (
  ...
  const url = `${API_POKEMONS}/${
    encodeURIComponent(name.trim())
  }?offset=${API_OFFSET}&limit=${API_LIMIT}`;

  try {
    const response: IPokemon = await getApiPokemonByName(url, reply.getHeaders());
    const {
      id,
      height,
      base_experience,
      sprites: {
        front_default: sprite_img
      } = {},
      species,
      stats
    } = response;
...
export const computeResponse = async (
  ...
  return response.stats.reduce((prev, stat) => prev + stat.base_stat, 0) / response.stats.length;
}
